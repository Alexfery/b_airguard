...
let SensorsController = class SensorsController {
    ...
    getHistory(deviceId, from, to) {
        if (!from || !to) {
            throw new common_1.BadRequestException('Query params "from" and "to" are required (ISO 8601)');
        }
        const fromDate = new Date(from);
        const toDate = new Date(to);
        if (isNaN(fromDate.getTime()) || isNaN(toDate.getTime())) {
            throw new common_1.BadRequestException('Invalid date format — use ISO 8601');
        }
